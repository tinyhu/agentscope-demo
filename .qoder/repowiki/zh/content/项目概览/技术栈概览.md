...
## 引言
本技术栈概览面向新加入的开发者，聚焦 AgentScope Demo 项目的技术选择与架构动机。项目基于 Spring Boot 3.5.14 + Java 17 构建，使用 AgentScope 2.0.2 GA 作为 AI Agent 框架的核心能力层，结合 Project Reactor 提供响应式 SSE 流式交互，利用 Apache POI 与 PDFBox 实现文档处理能力，并集成 DashScope API 提供智能对话能力。本文将围绕这些关键技术点展开说明，解释为何采用响应式架构、微内核设计和配置驱动等决策，并给出依赖关系、兼容性建议及常见问题的排查方向。
...
graph TB
    ...
    Runtime --> AgentScopeCore["AgentScope Core"]
...
- AI Agent 框架：AgentScope 2.0.2 GA（统一模型抽象、事件流、RAG、多模态、多智能体协作、Harness 等）
...
- 为什么选择响应式架构处理高并发 SSE 流？
  ...
  - AgentScope 2.0 原生提供事件流 streamEvents()，与 Project Reactor 无缝融合，便于统一编排与扩展（如审批、审计、度量等中间件）。
...
graph LR
    ...
    SB --> THY["Thymeleaf"]
    AG["AgentScope 2.0.2 GA"] --> CORE["agentscope-core"]
...
## 结论
该项目以 Spring Boot + Java 17 为基础，借助 AgentScope 2.0.2 GA 的统一 Agent 抽象与扩展能力，结合 Project Reactor 实现高性能、可观测的流式对话系统；通过 Apache POI 与 PDFBox 覆盖主流办公文档；以 DashScope 作为模型提供商，提供强大语言与推理能力。整体采用配置驱动与 Profile 扩展的微内核设计，使得系统在功能与部署上均具备良好弹性，适合快速孵化 AI 应用场景与后续生产演进。
...
- 基线版本
  ...
  - Java 17
  - AgentScope 2.0.2 GA（core、spring-boot-starter、harness、model-dashscope、rag-simple、memory-bailian、a2a-server/client/agui、redis/mysql/postgresql 扩展）
