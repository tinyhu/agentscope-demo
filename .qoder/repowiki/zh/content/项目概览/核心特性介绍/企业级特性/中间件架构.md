...
## 更新摘要
**所做更改**
- 新增了 final-answer-filter 中间件的完整文档说明
- 更新了中间件注册流程，包含最新的内置中间件列表
- 添加了最终答案过滤功能的详细解释和使用示例
- 更新了配置示例，展示如何在实际场景中使用该中间件
...
## 引言
本文件针对 AgentScope 2.0 的中间件（Middleware）机制，系统化阐述其在当前代码库中的设计与实现。重点涵盖 MiddlewareRegistry 的注册模式与工厂机制、生命周期钩子、内置中间件的发现与自动注册、执行链编排与错误传播、以及扩展开发指南和配置方式。读者将理解如何通过"洋葱圈"拦截层对 Agent 的全生命周期进行无侵入式增强（审计、限流、上下文注入、审批、指标收集、最终答案过滤等），并安全地扩展新能力。

**更新** 在 AgentScope 2.0.3 版本中新增了 final-answer-filter 中间件，用于抑制 ReAct 推理过程中的中间文本输出，仅向用户展示最终答案。
...
MR --> M6["FinalAnswerFilterMiddleware"]
MR --> M7["OtelTracingMiddleware"]
...
**更新** 新增了 FinalAnswerFilterMiddleware，用于过滤 ReAct 推理过程中的中间文本输出。
...
### 中间件注册与装配流程
- 自动注册：MiddlewareRegistry 使用 Spring PostConstruct 方法，将审计、限流、上下文注入、详细审计、指标收集、OpenTelemetry 追踪、最终答案过滤等内置中间件按命名注册到内部有序映射中。
...
**更新** 新增了 final-answer-filter 中间件的自动注册，位于注册表的第7个位置。
...
PostC --> RegG["注册 'final-answer-filter'"]
...
**更新** FinalAnswerFilterMiddleware 在推理阶段工作，过滤中间推理文本，只保留最终答案。
...
**更新** FinalAnswerFilterMiddleware 通过过滤中间推理文本减少网络传输和前端渲染开销，提升用户体验。
...
- 最终答案过滤无效：确认 final-answer-filter 中间件已正确注册，并检查其在中间件链中的位置。

**更新** 新增了 final-answer-filter 中间件的故障排查指导。
...
## 结论
本项目在 AgentScope 2.0 的基础上实现了稳健的中间件体系：通过统一的注册中心与装配入口，结合可插拔的生命周期钩子，使审计、限流、上下文增强、审批、指标收集、分布式追踪和最终答案过滤等功能得以解耦与复用。该架构具备良好的可扩展性与清晰的职责边界，便于未来继续丰富中间件生态。

**更新** 新增的 final-answer-filter 中间件进一步增强了用户体验，通过抑制中间推理过程的不必要输出，提供更简洁、专业的交互界面。
...
- **FinalAnswerFilterMiddleware**：抑制 ReAct 流式输出中中间推理轮次的文本，仅输出最终答案（AgentScope 2.0.3 新增）

**更新** 新增了 FinalAnswerFilterMiddleware 的详细说明。
...
**更新** 提供了包含 final-answer-filter 的完整配置示例。
...
### 最终答案过滤中间件使用示例
以下是在实际配置中使用 final-answer-filter 中间件的示例：
...
**更新** 新增了 final-answer-filter 中间件的实际配置示例和使用说明。

**章节来源**
- [agents.yml:1760-1780](file://src/main/resources/config/agents.yml#L1760-L1780)