...
- ModelName：具体推理模型名称（如 qwen-plus、deepseek-v4-flash-0731），默认值在代码中设置；application.yml 也可提供全局默认 provider 与 modelName
...
- modelName
  - 代码默认值：qwen-plus；示例 YAML 中多采用 deepseek-v4-flash-0731
...
- modelName 切换需关注不同模型的吞吐与价格；默认与全局配置保持一致有助于简化排障
...
### 最佳实践清单
- 统一 modelName 策略：在 application.yml 设定全局默认，个别 Agent 差异化覆盖
