...
## 更新摘要
**变更内容**
- 更新了默认模型配置：从 deepseek-v4-flash-0731 升级到 deepseek-v4.1-flash
- 更新了部分Agent的模型配置：chat-basic Agent使用 qwen3.8-flash 模型
- 修正了相关文档中的模型版本引用和说明
...
- 模型ID解析策略：
  ...
  - 不含冒号时使用 dashscope: 前缀，避免 deepseek-v4.1-flash 等无法匹配提供方命名模式的场景。
- 上下文装配：
...
- 配置位置：
  - application.yml 的 agentscope.model.dashscope.* 定义了：
    ...
    - model-name：**已更新为 deepseek-v4.1-flash**（之前为 deepseek-v4-flash-0731）。
...
**更新** 默认模型已从 deepseek-v4-flash-0731 升级到 deepseek-v4.1-flash，提供更好的性能和成本效益。
...
### 多模型支持与扩展
- 多模型选择策略：
  - **已更新** 默认模型：application.yml 中 dashscope.model-name 现在为 **deepseek-v4.1-flash**（之前为 deepseek-v4-flash-0731）。
  - **新增** Agent 级别覆盖：agents.yml 中 chat-basic Agent 使用 **qwen3.8-flash** 模型，其他多个 Agent 使用 deepseek-v4.1-flash。
...
**更新** 项目现在采用更现代化的模型组合策略，基础对话使用 qwen3.8-flash，复杂任务使用 deepseek-v4.1-flash，以获得更好的性能和成本平衡。
...
- 模型解析错误：
  - 若使用无前缀的模型名（如 deepseek-v4.1-flash），确保其会被自动加上 dashscope: 前缀；必要时显式传入带前缀的 ID。
...
**更新** 由于默认模型已升级到 deepseek-v4.1-flash，如果遇到兼容性问题，可以回退到之前的 deepseek-v4-flash-0731 或在特定 Agent 中使用 qwen3.8-flash。
...
## 结论
本项目通过 ModelFactory 实现对 AgentScope 2.0 的 ModelRegistry 的统一适配，将模型创建、Provider 前缀处理、API Key 管理与流式/思维模式配置集中在一个高内聚的组件内。配合 application.yml 的集中配置与 agents.yml 的细粒度覆盖，既能满足多模型、多 Agent 的灵活需求，又保持了运行时代码的低耦合与一致性。

**更新** 随着默认模型升级到 deepseek-v4.1-flash 和部分 Agent 使用 qwen3.8-flash，项目在性能和成本之间取得了更好的平衡。对于生产环境，建议在此基础上补充统一的超时与重试策略、完善监控埋点，并结合业务特性持续优化上下文长度与采样参数，以达到更好的响应速度、成本控制与用户体验。